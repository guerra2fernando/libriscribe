...
from libriscribe.knowledge_base import Chapter, ProjectKnowledgeBase  # Import the new class
...
def write_and_review_chapters(project_knowledge_base: ProjectKnowledgeBase):
    ...
    for i in range(1, num_chapters + 1):
        ...
        if chapter is None:
            ...
            chapter = Chapter(chapter_number=i, title=f"Chapter {i}", summary="To be written")
...
def get_advanced_fiction_details(project_knowledge_base: ProjectKnowledgeBase):
    ...
    target_audience = select_from_list(
        "👥 Who is your target audience?", ["Children", "Teens", "Young Adult", "Adults"]
    )
    ...
    num_chapters_str = typer.prompt(
        "📑 Approximately how many chapters do you want? (e.g., 10, 8-12, 20+)", default="8-12"
    )
