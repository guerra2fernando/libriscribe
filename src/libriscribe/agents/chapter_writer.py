...
class ChapterWriterAgent(Agent):
    ...
    def execute(
        ...
        try:
            ...
            if not chapter:
                console.print(
                    f"[red]ERROR: Chapter {chapter_number} not found in knowledge base.[/red]"
                )
                ...
                console.print(
                    f"[yellow]Created default chapter {chapter_number} to proceed.[/yellow]"
                )
            ...
            for scene in ordered_scenes:
                console.print(
                    f"🎬 Creating Scene/Section {scene.scene_number} of {len(ordered_scenes)}..."
                )

                # Generate a scene title if none exists
                scene_title = (
                    f"Scene {scene.scene_number}: {scene.summary[:30]}..."
                    if len(scene.summary) > 30
                    else f"Scene {scene.scene_number}: {scene.summary}"
                )
                ...
                scene_prompt = prompts.SCENE_PROMPT.format(
                    ...
                    characters=(
                        ", ".join(scene.characters) if scene.characters else "None specified"
                    ),
                    setting=scene.setting if scene.setting else "None specified",
                    goal=scene.goal if scene.goal else "None specified",
                    emotional_beat=(
                        scene.emotional_beat if scene.emotional_beat else "None specified"
                    ),
                ...
                if not scene_content:
                    console.print(
                        f"[yellow]Warning: Failed to generate content for Scene {
    scene.scene_number}. Using placeholder.[/yellow]"
                    )
                    scene_content = f"[Scene {scene.scene_number} content unavailable]"

                # Ensure the scene title is included
                if not scene_content.startswith(
                    f"**{scene_title}**"
                ) and not scene_content.startswith(f"# {scene_title}"):
            ...
            console.print(
                f"[green]✅ Chapter {chapter_number} completed with {
    len(ordered_scenes)} scenes![/green]"
            )
