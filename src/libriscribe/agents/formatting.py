    ...
    def execute(self, project_dir: str, output_path: str) -> None:
        ...
        try:
            ...
            prompt = prompts.FORMATTING_PROMPT.format(
                chapters=all_chapters_content, language=project_knowledge_base.language
            )
            formatted_markdown = self.llm_client.generate_content(
                prompt, max_tokens=120000
            )  # May need large token limit
